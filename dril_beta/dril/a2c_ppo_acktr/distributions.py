...
class DiagBeta(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.alpha_head = init_(nn.Linear(num_inputs, num_outputs))
        self.beta_head  = init_(nn.Linear(num_inputs, num_outputs))
        self.sp = nn.Softplus()

    def forward(self, x):
        alpha = self.sp(self.alpha_head(x)) + 1
        beta = self.sp(self.beta_head(x)) + 1
...
# CHECKS DRIL BETA DISTRIBUTION


def test_1():
    input_th = torch.ones([8])
    input_th = input_th.unsqueeze(0)
    beta_dist = DiagBeta(8,1)
    dist_curve, alpha, beta = beta_dist(input_th)
...
def test_2():
    input_th = torch.ones([8])
    input_th = input_th.unsqueeze(0)
    beta_dist = DiagBeta(8, 2)
    dist_curve, alpha, beta = beta_dist(input_th)

    output = beta_dist.get_mean(input_th)
    print(alpha)
    print(beta)
    print(output)
