...
class SplitBias(nn.Module):
    def __init__(self, module):
        ...
        self.module = module
        self.add_bias = AddBias(module.bias.data_beta)
...
class KFACOptimizer(optim.Optimizer):
    ...
    def _save_input(self, module, input):
        if torch.is_grad_enabled() and self.steps % self.Ts == 0:
            ...
            aa = compute_cov_a(input[0].data_beta, classname, layer_info,
                               self.fast_cnn)
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if self.acc_stats:
            ...
            gg = compute_cov_g(grad_output[0].data_beta, classname, layer_info,
                               self.fast_cnn)
    ...
    def step(self):
        ...
        if self.weight_decay > 0:
            for p in self.model.parameters():
                p.grad.data_beta.add_(self.weight_decay, p.data_beta)
        ...
        for i, m in enumerate(self.modules):
            ...
            if classname == 'Conv2d':
                p_grad_mat = p.grad.data_beta.view(p.grad.data_beta.size(0), -1)
            else:
                p_grad_mat = p.grad.data_beta
            ...
            v = v.view(p.grad.data_beta.size())
            updates[p] = v

        vg_sum = 0
        for p in self.model.parameters():
            v = updates[p]
            vg_sum += (v * p.grad.data_beta * self.lr * self.lr).sum()

        nu = min(1, math.sqrt(self.kl_clip / vg_sum))

        for p in self.model.parameters():
            v = updates[p]
            p.grad.data_beta.copy_(v)
            p.grad.data_beta.mul_(nu)
