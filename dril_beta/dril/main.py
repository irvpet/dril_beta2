...
from dril.a2c_ppo_acktr.algo.behavior_cloning import DISTRIBUTION
...
def main():
    ...
    if args.behavior_cloning or args.dril or args.warm_start:
        ...
        bc_model_path = os.path.join(bc_model_save_path, f'{bc_file_name}.{DISTRIBUTION}.model.pth')
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            if DISTRIBUTION == 'Beta':
                action_rescale = action *2. -1.
            else:
                action_rescale = action

            # Obser reward and next obs
            if isinstance(envs.action_space, gym.spaces.Box):
                clip_action = torch.clamp(action_rescale, float(envs.action_space.low[0]), float(envs.action_space.high[0]))
            else:
                clip_action = action_rescale
        ...
        if (j % args.save_interval == 0
            ...
            model_file_name = f'{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}.{DISTRIBUTION}.Iter={j}'
            torch.save([
                actor_critic,
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            file_name = f'{algo}_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}{DISTRIBUTION}iter={j}'
...
def save_results(args, main_results, algo, dril, gail):
    ...
    if dril:
        ...
        exp_name += f'cost_{args.dril_cost_clip}_seed={args.seed}{DISTRIBUTION}.perf'
    elif gail:
        exp_name  = f'{algo}_{args.env_name}_ntraj={args.num_trajs}_'
        ...
        exp_name  = f'{algo}_{args.env_name}{DISTRIBUTION}.pef'
